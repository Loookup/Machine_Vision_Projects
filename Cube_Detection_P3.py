...
def Circle_Detection(img, prev_centroids, prev_rvec):
    ...
    corners = np.zeros((1, 3, 2), dtype=np.float64)
    ...
    if ret == 4:
        ...
        # pos = -1
        # rvec_rev = np.array([rvec[pos][0][0], rvec[pos][1][0], rvec[pos][2][0]])
        # tvec_rev = np.array([tvec[pos][0][0], tvec[pos][1][0], tvec[pos][2][0]])

        # imgpt_rev, jac = cv2.projectPoints(point_4, rvec_rev, tvec_rev, mtx, dist)

        # img_point_4_rev = tuple(imgpt_rev[0].ravel())

        # dist_list = []

        # criterion = cv2.norm(corners[0][0], corners[0][2])
        ...
        # for i in range(0, len(rvec)):
            ...
            # dist_1 = cv2.norm(img_point_4, corners[0][0])
            # dist_2 = cv2.norm(img_point_4, corners[0][2])
            # dist_list.append(criterion*criterion - dist_1*dist_1 - dist_2*dist_2)

        # pos2 = np.argmin(dist_list)

        # print(dist_list)

        # print(pos2)

        # R = np.array([rvec[pos2][0][0], rvec[pos2][1][0], rvec[pos2][2][0]])
        # T = np.array([tvec[pos2][0][0], tvec[pos2][1][0], tvec[pos2][2][0]])
        ...
        return ret, img, corners, R, T, prev_centroids, prev_rvec

    else: return ret, img, corners, None, None, prev_centroids, prev_rvec
...
while True:
    ...
    ret2, frame, corners, rvec, tvec, prev_centroids, prev_rvec = Circle_Detection(frame, prev_centroids, prev_rvec)

    if detect == True and ret2 == 4:
