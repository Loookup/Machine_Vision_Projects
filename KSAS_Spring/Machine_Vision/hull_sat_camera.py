...
class Vertex():
    ...
    def Watershed(self, img):
        ...
        ret, thresholded = cv2.threshold(gray, 14, 255, cv2.THRESH_BINARY)
    ...
    def Hull(self, img, centroids):

        new_centroids = numpy.zeros((8, 2), dtype=numpy.uint16)

        for idx in range(8):

            new_centroids[idx] = centroids[idx+1].astype(int)

        hull = ConvexHull(new_centroids)

        outer_centroids = numpy.zeros((5, 2), dtype=numpy.uint16)

        for i in range(4):

            outer_centroids[i+1] = new_centroids[hull.vertices[i]]

        outer_centroids = self.Switching(outer_centroids, outer_centroids)

        for idx in range(4):

            cv2.putText(img, str(idx+1), tuple(outer_centroids[idx+1]), cv2.FONT_HERSHEY_DUPLEX, 2, (0, 0, 255), 2)
